...
int set_speed(float speed) {
  ...
  uint8_t cmd[len] = {0x84, 0x00, 0x00, 0x00}; // 0x84 == set target
  uint16_t mc_speed = uint16_t(speed*100+6040);
...
/*We get an angle value between -22 and 22 from joystick.
  ...
  uint16_t position = uint16_t(angle*45+6000);
...
/*
  Sends a command of 4 bytes to the motor controller over serial.
  (Appends a checksum)
  byte 1: Command byte - Use 0x84 for set target
  byte 2: Channel byte - Number of the channel to send command to
  byte 3: Lower Target byte
  byte 4: Higher Target byte
*/
int send_packet(const unsigned char *data, int len) {
  unsigned char buffer[len];
  //For old motorcontroller
  //buffer[0] = 2;
  //buffer[1] = len;
